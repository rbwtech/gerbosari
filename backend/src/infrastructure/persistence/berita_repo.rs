...
impl BeritaRepository for MysqlBeritaRepository {
    ...
    async fn get_by_slug(&self, slug: &str) -> Result<Berita, AppError> {
        let row: Option<BeritaRow> = sqlx::query_as::<_, BeritaRow>(
            "SELECT id, judul, slug, kategori, ringkasan, konten, gambar_url, tanggal_acara, \
             author, published_at FROM berita WHERE slug = ?",
        )
        .bind(slug)
